...
const { version, dependencies } = await fs.readJSON(
...
function help() {
	...
	const dashCount = bin.length + version.length + 35;
	const dashes = new Array(dashCount).fill("-").join("");

	console.log(`${dashes}
${chalk.green.bold(
	bin.toLocaleUpperCase()
)} - the Playwright Prerender CLI ${chalk.dim(`(v${version})`)}
${dashes}
